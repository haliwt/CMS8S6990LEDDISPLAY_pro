...
{
	ADCON0 &= ~(ADC_ADCON0_ADCHS4_Msk);
	ADCON1 &= ~(ADC_ADCON1_ADCHS_Msk);
		
	ADCON0 |= ((ADCChannel & 0x10)<<3);
	ADCON1 |= ADCChannel & 0xf;	
}
...
{
	ADCON0 &= ~(ADC_ADCON0_AN31SEL_Msk);
	ADCON0 |= (An31Channel<<ADC_ADCON0_AN31SEL_Pos);
