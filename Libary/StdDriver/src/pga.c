...
{
	PGACON3 &= ~(PGA_PAGACON3_PGAMODE_Msk);
	PGACON0 &= ~(PGA_PAGACON0_PGAMS_Msk);
	PGACON0 |= PGAMode<<PGA_PAGACON0_PGAMS_Pos;
...
{
	PGACON0 &= ~(PGA_PAGACON0_PGAMC_Msk);
	PGACON0 |= Port;
}
...
{
	PGACON2 |= PGA_PAGACON2_PGATEN_Msk;
}
/*****************************************************************************
...
{
	PGACON0 &= ~(PGA_PAGACON0_PGAGS_Msk);
	PGACON0 |= Gain<<PGA_PAGACON0_PGAGS_Pos;
}
...
{
	PGACON3 &= ~(PGA_PAGACON3_PGAADJ_Msk);
	PGACON3 |= AdjVlue;
	PGAADJE = OffsetAdj;
}
...
{
	PGACON1 &= ~(PGA_PAGACON1_PGASHEN_Msk);
}
...
{
	PGACON1 &= ~(PGA_PAGACON1_PGASHT_Msk);
	PGACON1 |= SampleTime;
}
...
void PGA_Start(void)
{
	PGACON0 |= PGA_PAGACON0_PGAEN_Msk;
}
/*****************************************************************************
 ** \brief	 PGA_Stop
 **			 关闭PGA
...
{
	return((PGACON2 & PGA_PAGACON2_PGADOUT_Msk)? 1:0);
}
